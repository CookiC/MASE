...
public class NSGAII extends ASSearch implements StartSetHandler, OptionHandler, TechnicalInformationHandler {
	...
	private void populationStatistics() {
		...
		m_avgFitness = m_sumFitness.divide(m_popSize / 2);
	}
	 ...
	 * 
	 */
	public class PopComparator implements Comparator<GABitSet> {
		int obj;
	...
	public int[][] search(WrapperSubsetEval ASEval, Instances data, String[] objs) throws Exception {
		...
		for (int i = 1; i <= m_maxGenerations; i++) {
			...
			/*
			 * System.out.println("");
			 * System.out.println("This is the "+i+"th generation");
			 * System.out.print("m_population[ 0]: rank=" + m_population[0].rank
			 * +", chromosome=" + m_population[0].getChromosome()+"  "); for
			 * (int jj = 0; jj != m_objects; ++jj)
			 * System.out.print("objective"+jj+"={"+m_population[0].getFitness()
			 * .d[jj] + "  " + m_population[0].getObjective().d[jj]+"}  "); for
			 * (int j = 0; j != m_popSize; ++j) {
			 * System.out.print("m_population["); if(j<10)
			 * System.out.print(" "); System.out.print(j+"]: rank=" +
			 * m_population[j].rank +", chromosome=" +
			 * m_population[j].getChromosome()+"  "); for (int jj = 0; jj !=
			 * m_objects; ++jj)
			 * System.out.print("objective"+jj+"={"+m_population[j].getFitness()
			 * .d[jj] + "  " + m_population[j].getObjective().d[jj]+"}  ");
			 * System.out.println(); }
			 */
		}
