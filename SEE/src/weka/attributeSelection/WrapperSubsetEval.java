...
public class WrapperSubsetEval extends ASEvaluation
   ...
   */
    ...
    public double[] evaluateSubset(BitSet subset,String[] statNames) throws Exception {
	    ...
	    double[][] repError = new double[numObjective][5];
	    Random Rnd = new Random(m_seed);
	    BitSet subsetCopy=(BitSet)subset.clone();
	    subsetCopy.set(m_classIndex);
	    Instances trainCopy = new Instances(m_trainInstances.attributeFilter(subsetCopy));
	    ...
	    for (j = 0; j < 5; j++) {
	    	...
	    	for(i = 0; i < numObjective; ++i){
	    		...
	    		switch (m_evaluationMeasure.getID()) {
			  		case EVAL_DEFAULT:
			  			repError[i][j] = m_Evaluation.errorRate();
			  			break;
			  		case EVAL_ACCURACY:
			  			repError[i][j] = m_Evaluation.errorRate();
			  			break;
			  		case EVAL_RMSE:
			  			repError[i][j] = m_Evaluation.rootMeanSquaredError();
			  			break;
			  		case EVAL_MAE:
			  			repError[i][j] = m_Evaluation.meanAbsoluteError();
			  			break;
			  		case EVAL_FMEASURE:
			  			if (m_IRClassVal < 0)
			  				repError[i][j] = m_Evaluation.weightedFMeasure();
			  			else
			  				repError[i][j] = m_Evaluation.fMeasure(m_IRClassVal);
			  			break;
			  		case EVAL_AUC:
			  			if (m_IRClassVal < 0)
			  				repError[i][j] = m_Evaluation.weightedAreaUnderROC();
			  			else
			  				repError[i][j] = m_Evaluation.areaUnderROC(m_IRClassVal);
			  			break;
			  		case EVAL_AUPRC:
			  			if (m_IRClassVal < 0)
			  				repError[i][j] = m_Evaluation.weightedAreaUnderPRC();
			  			else
			  				repError[i][j] = m_Evaluation.areaUnderPRC(m_IRClassVal);
			  			break;
			  		case EVAL_CORRELATION:
			  			repError[i][j] = m_Evaluation.correlationCoefficient();
			  			break;
			  		default:
			  			if (m_evaluationMeasure.getID() >= EVAL_PLUGIN) {
			  				metricName = ((PluginTag) m_evaluationMeasure).getMetricName();
			  			...
			  			if (pluginMetric instanceof InformationRetrievalEvaluationMetric) {
			  				if (m_IRClassVal < 0)
			  					repError[i][j] = ((InformationRetrievalEvaluationMetric) pluginMetric).getClassWeightedAverageStatistic(statName);
			  				else 
			  					repError[i][j] = ((InformationRetrievalEvaluationMetric) pluginMetric).getStatistic(statName, m_IRClassVal);
			  			} else
			  				repError[i][j] = pluginMetric.getStatistic(statName);
			  			break;
	    		}
	    		terminate&=!repeat(repError[i], j + 1);
	    	}
	  ...
	  m_Evaluation = null;
	  for(k=0;k<numObjective;++k){
		  for (i = 0; i < j; i++)
			  evalMetric[k] += repError[k][i];
		  evalMetric[k] /= j;
		  ...
		  switch (m_evaluationMeasure.getID()) {
		  	...
		  	case EVAL_MAE:
		  		if (m_trainInstances.classAttribute().isNominal()&& (m_evaluationMeasure.getID() == EVAL_DEFAULT|| m_evaluationMeasure.getID() == EVAL_ACCURACY))
		  			evalMetric[k] = 1 - evalMetric[k];
		  		else
		  			evalMetric[k] = -evalMetric[k]; // maximize
		  		break;
		  	default:
  ...
  protected double getRepError(int ID) throws Exception{
	  double repError;
	  AbstractEvaluationMetric pluginMetric = null;
	  String statName = null;
	  String metricName = null;
	  switch (ID) {
      	case EVAL_DEFAULT:
      		repError = m_Evaluation.errorRate();
      		break;
      	case EVAL_ACCURACY:
      		repError = m_Evaluation.errorRate();
      		break;
      	case EVAL_RMSE:
      		repError = m_Evaluation.rootMeanSquaredError();
      		break;
      	case EVAL_MAE:
      		repError = m_Evaluation.meanAbsoluteError();
      		break;
      	case EVAL_FMEASURE:
      		if (m_IRClassVal < 0)
      			repError = m_Evaluation.weightedFMeasure();
      		else
      			repError = m_Evaluation.fMeasure(m_IRClassVal);
      		break;
      	case EVAL_AUC:
      		if (m_IRClassVal < 0)
      			repError = m_Evaluation.weightedAreaUnderROC();
      		else
      			repError = m_Evaluation.areaUnderROC(m_IRClassVal);
      		break;
      	case EVAL_AUPRC:
      		if (m_IRClassVal < 0)
      			repError = m_Evaluation.weightedAreaUnderPRC();
      		else
      			repError = m_Evaluation.areaUnderPRC(m_IRClassVal);
      		break;
      	case EVAL_CORRELATION:
      		repError = m_Evaluation.correlationCoefficient();
      		break;
      	default:
      		if (m_evaluationMeasure.getID() >= EVAL_PLUGIN) {
      			metricName = ((PluginTag) m_evaluationMeasure).getMetricName();
      		...
      		if (pluginMetric instanceof InformationRetrievalEvaluationMetric) {
      			if (m_IRClassVal < 0) {
      				repError = ((InformationRetrievalEvaluationMetric) pluginMetric)
      			...
      			repError = pluginMetric.getStatistic(statName);
      		}
      		break;
      }
	  return repError;
    ...
    protected boolean statisticIsMaximisable(int ID) throws Exception{
    	switch (ID) {
        	case EVAL_DEFAULT:
        	case EVAL_ACCURACY:
        	case EVAL_RMSE:
        	case EVAL_MAE:
        		if (m_trainInstances.classAttribute().isNominal()&&(ID==EVAL_DEFAULT||ID==EVAL_ACCURACY))
        			return false;
        		else
